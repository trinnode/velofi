...
// Define Somnia Network (using testnet configuration)
...
// Define Somnia Mainnet for future use
...
export const config = createConfig({
  chains: [somniaTestnet, somniaMainnet, mainnet, sepolia],
  connectors: [
    injected(),
    metaMask(),
    walletConnect({
  ...
  transports: {
    ...
    [somniaMainnet.id]: http("https://rpc.somnia.network"),
    [mainnet.id]: http(),
    [sepolia.id]: http(),
  },
  ssr: true,
...
export const CONTRACT_ADDRESSES = {
  [somniaTestnet.id]: {
    ...
    CREDIT_SCORE: process.env
      .NEXT_PUBLIC_CREDIT_SCORE_CONTRACT as `0x${string}`,
    ...
    LIQUIDITY_POOL: process.env
      .NEXT_PUBLIC_LIQUIDITY_POOL_CONTRACT as `0x${string}`,
...
export const API_ENDPOINTS = {
  BASE_URL:
    process.env.NEXT_PUBLIC_API_URL || "https://dream-rpc.somnia.network",
  WEBSOCKET_URL:
...
export const FEATURES = {
  ...
  CREDIT_SCORING_ENABLED:
    process.env.NEXT_PUBLIC_CREDIT_SCORING_ENABLED === "true",
} as const;
