...
export default function CreditPage() {
  ...
  useEffect(() => {
    ...
    if (isConnected && address) {
      trackEvent('credit_wallet_connected', { address })
    }
  }, [isConnected, address])
                ...
                <div className="text-center mb-8">
                  <div className="text-6xl font-bold mb-2 flex items-center justify-center gap-4">
                    <span className={getCreditScoreColor(creditData.userScore)}>
                      {creditData.userScore !== undefined ? creditData.userScore : (
                  ...
                  {creditData.userScore === undefined && (
                    <div className="text-center text-neon-magenta font-semibold mt-2">Unable to fetch credit score. Please try again later.</div>
                  )}
                ...
                <div className="space-y-4">
                  {creditData.recentActivity?.length > 0 ? (
                    creditData.recentActivity.map((activity) => (
                      <div key={activity.action + activity.date} className="flex items-center gap-4 p-4 bg-jet-black/30 rounded-lg border border-gray-700">
